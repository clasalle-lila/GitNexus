...
| `GITNEXUS_INDEX_HIDDEN`                         | `0` (excluded)            | When `1`/`true`, the walker enumerates hidden (dot-prefixed) paths. `DEFAULT_IGNORE_LIST` still applies, so `.git/` and `node_modules/` stay excluded; narrow the rest with `.gitnexusignore`.                                                                                                                | Repos that keep first-party source under a dot-directory. Without it those paths are never enumerated, so `.gitnexusignore` negation cannot re-include them and analyze reports success while skipping them. |
